...
// All other Qs are treated as Analytical to prevent silent dropping
...
function classifyNarrative(text: string | undefined) {
  if (!text || !text.trim()) return 'incomplete';
  // A 'complete' response in GDC standards usually needs more depth than 120 chars
  if (text.trim().length < 200) return 'needs-review';
  return 'complete';
}

function findEvidenceSignals(
...
function inspectionQuestionsForRequirement(reqId: string): string[] {
...
export function analyzeQuestionnaire(
  ...
  for (const q of questions) {
    ...
    // Process all analytical questions (including those missing from the set)
